...
def alipay_notify():
    ...
    try:
        ...
        current_app.logger.info(f"Received Alipay notification: {data}")

        # 验证通知的真实性
        if not AlipayService.verify_async_notification(data):
        ...
        # 检查必要参数
        if not all([out_trade_no, trade_status]):
            current_app.logger.warning("Missing required parameters in Alipay notification")
            return "failure", 400
        ...
        current_app.logger.error(f"Error handling Alipay notification: {str(e)}")
        return "failure", 500
